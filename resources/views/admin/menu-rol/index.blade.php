              ...
              <div class="card-body table-responsive p-0">
                ...
                <table class="table table-striped table-bordered table-hover" id="tabla-data">
                    <thead>
                        <tr>
                        <th>Menús</th>
                        @foreach($rols as $id => $nombre_rol)
                        <th class="text-center">{{$nombre_rol}}</th>
                        @endforeach
                    </thead>
                    <tbody>
                        @foreach($menus as $key =>$menu)
                            ...
                            @endif
                            <tr>
                                <td class="font-weight-bold"><i class="fas fa-caret-square-down"></i> {{$menu["nombre"]}}</td>
                            ...
                            @foreach($menu["submenu"] as $key => $hijo)
                                <tr>
                                    <td class="pl-40"><i class="fas fa-caret-square-right"></i> {{$hijo["nombre"]}}</td>
                                    @foreach($rols as $id =>$nombre)
                                        <td class="text-center">
                                            <input
                                            type="checkbox"
                                ...
                                @foreach($hijo["submenu"] as $key => $hijo2)
                                    <tr>
                                        <td class="pl-60"><i class="fas fa-caret-square-right"></i> {{$hijo2["nombre"]}}</td>
                                        @foreach($rols as $id =>$nombre)
                                            <td class="text-center">
                                                <input
                                                type="checkbox"
                                    ...
                                    @foreach($hijo2["submenu"] as $key => $hijo3)
                                        <tr>
                                            <td class="pl-80"><i class="fas fa-caret-square-right"></i> {{$hijo3["nombre"]}}</td>
                                            @foreach($rols as $id =>$nombre)
                                                <td class="text-center">
                                                    <input
                                                    type="checkbox"
